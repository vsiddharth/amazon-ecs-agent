...
type UdevWatcher struct {
	...
	events               chan *enimonitor.ENIEvent
...
func newWatcher(ctx context.Context,
	...
	return &UdevWatcher{
		...
		events:         make(chan *enimonitor.ENIEvent),
		agentState:     state,
...
func (udevWatcher *UdevWatcher) reconcileOnce() error {
	interfaces, err := udevWatcher.netlinkClient.Interfaces()
	if err != nil {
	...
	currentState := udevWatcher.buildState(interfaces)

	// NOTE: For correct semantics, this entire function needs to be locked.
	// As we postulate the netlinkClient.Interfaces() call to be expensive, we allow
...
func (udevWatcher *UdevWatcher) buildState(interfaces []net.Interface) map[string]string {
	state := make(map[string]string)
	for _, nwinterface := range interfaces {
		if (nwinterface.Flags & net.FlagUp) == 0  && (nwinterface.Flags & net.FlagLoopback) != 0 {
			// We only care about UP and non loopback interfaces. Ignore other interfaces.
			continue
		}

		macAddress := nwinterface.HardwareAddr.String()
		if macAddress != "" && macAddress != udevWatcher.primaryMAC {
			state[macAddress] = nwinterface.Name
		}
	}
	return state
}
...
func (udevWatcher *UdevWatcher) eventHandler() {
	...
	for {
		...
		case event := <-udevWatcher.events:
			log.Debugf("Udev watcher event-handler: Sending ENI state change for MACAddress:%v", event.HardwareAddr)
			if err := udevWatcher.sendENIStateChangeWithRetries(udevWatcher.ctx, event.HardwareAddr, sendENIStateChangeRetryTimeout); err != nil {
